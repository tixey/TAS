...
<head>
    <title>TAON Company Limited - Bills</title>
    ...
    <script>
        ...
        const loadTool = () => { 
            ...
            const loadStores = () => Stores.load([{name:'currencies'},{name:'counterPartiesGrid'},{name:'billsGrid'}])
            const loadMainLayout = () => Components.load([mainLayout])
            const loadComponents = () => Components.load([mainMenu,billsGrid])
            loadMainLayout()
                .then(loadStores)
                .then(loadComponents)
        }
